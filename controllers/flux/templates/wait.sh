...
which sudo > /dev/null || (echo "sudo is required to be installed" && exit 1);
which flux > /dev/null || (echo "flux is required to be installed" && exit 1);
...
sudo adduser --disabled-password --uid 1000 --gecos "" flux >> /dev/null || {{ if not .TestMode }} printf "flux user is already added.\n"{{ else }}true{{ end }}
...
{{ if not .TestMode }}        echo "Extra arguments are: $@"{{ end }}
        if [ "$@" == "" ]; then
            ...
            python3 -m pip install -r requirements.txt > /dev/null || python -m pip install -r requirements.txt >> /dev/null
